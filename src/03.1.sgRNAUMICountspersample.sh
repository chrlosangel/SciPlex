#!/bin/bash
#SBATCH --job-name=SciPlex_sgRNA_UMICounts
...
SGRNA_OUTPUT_FILE=$output_dir/${sample}_sgRNA_UMI_counts.txt
SGRNA_OUTPUT_FILE_WITH_MAP=$output_dir_wname/${sample}_sgRNA_UMI_counts_gene_mapped.txt 
...
echo "[INFO] Input file: $SGRNA_INPUT_FILE"
echo "[INFO] Output file: $SGRNA_OUTPUT_FILE"


# Run the pipeline on all files
# Removes duplicate (Cell_ID, sgRNA, UMI).?? 
     # Check if we actually have duplicates.
# Groups by (Cell_ID, sgRNA).
# Counts the number of unique UMIs for each (Cell_ID, sgRNA).

zcat "${SGRNA_INPUT_FILE}" | \
    awk '{print $1"_"$2, $4, $3}' |  \
    sort -u | awk '{print $1, $2}' | uniq -c | awk '{print $2, $3, $1}' > "${SGRNA_OUTPUT_FILE}"
